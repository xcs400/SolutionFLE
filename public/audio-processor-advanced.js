...
// Responsabilités : traitement du signal uniquement (DSP).
// Le scoring (scorePitch, scoreRhythm, scoreStress) est dans spell.html.
// ═══════════════════════════════════════════════════════════════
...
; (function (global) {
  ...
  function detectSyllables(env, pitchResult) {
    ...
    // Lissage léger — radius divisé par ~2 vs l'ancienne version
    const eSmooth = gaussSmooth(gaussSmooth(env, n * 0.012), n * 0.008);
    const vSmooth = gaussSmooth(voiced, n * 0.025);

    // Score combiné avec floor voiced (évite l'écrasement sur voix régulière)
    const combined = new Float32Array(n);
    for (let i = 0; i < n; i++) {
      combined[i] = Math.sqrt(
        Math.max(0, eSmooth[i]) * Math.max(0.15, vSmooth[i])
      );
    }
    ...
    const thr = globalMax * 0.13;
    const minGap = Math.max(4, Math.floor(n * 0.045));
  ...
  // Export : seulement la classe DSP, pas les fonctions de scoring
